...
public final class ProjectorScreen extends HandledScreen<ProjectorScreenHandler> {
	...
	protected void init() {
		...
		addButton(mOffsetZInput);

		addButton(new TexturedButtonWidget(leftPos + 117, topPos + 153, 18, 19, 179, 153, 0, GUI_TEXTURE, button -> {
			ProjectorBlock.InternalRotation newRotation = mRotation.flip();
			updateRotation(newRotation);
		}));
		addButton(new TexturedButtonWidget(leftPos + 142, topPos + 153, 18, 19, 179, 173, 0, GUI_TEXTURE, button -> {
		...
		addButton(mSwitchURL);
		addButton(mSwitchID);
		addButton(mSwitchContainer);
		addButton(mButtonExport);
		addButton(mSwitchSingleSided);
		addButton(mSwitchDoubleSided);
	}
