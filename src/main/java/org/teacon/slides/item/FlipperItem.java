...
public class FlipperItem extends Item {
    ...
    public static void setProjectorPos(ItemStack stack, @Nullable BlockPos pos) {
        ...
        if(pos == null) {
            stack.removeSubTag("projector");
            return;
        }
        stack.putSubTag("projector", new NbtIntArray(new int[]{pos.getX(), pos.getY(), pos.getZ()}));
    }

    public static boolean trySendFlip(World level, PlayerEntity player, ItemStack itemStack, boolean back, boolean init) {
    ...
    public TypedActionResult<ItemStack> use(World level, PlayerEntity player, Hand interactionHand) {
        ...
        if(trySendFlip(level, player, itemStack, false, player.isSneaking())){
    ...
    public void inventoryTick(ItemStack itemStack, World level, Entity entity, int slot, boolean selected) {
        ...
        if(level.isClient() && selected) {
            keyAttackDown = MinecraftClient.getInstance().options.keyAttack.isPressed();
    ...
    public static void handleServerFlipBack(MinecraftServer minecraftServer, ServerPlayerEntity serverPlayer, PacketByteBuf packet) {
        ...
        minecraftServer.execute(() -> {
            ItemStack itemStack = serverPlayer.inventory.getStack(i);
            if(!(itemStack.getItem() instanceof FlipperItem && trySendFlip(serverPlayer.getServerWorld(), serverPlayer, itemStack, true, false))){
