...
public class Slideshow implements ModInitializer {
	...
	public static final Block PROJECTOR_BLOCK = registerBlockAndItem("projector", new ProjectorBlock());
	public static BlockEntityType<ProjectorBlockEntity> PROJECTOR_BLOCK_ENTITY = Registry.register(Registry.BLOCK_ENTITY_TYPE, new Identifier(Slideshow.ID, "projector"), BlockEntityType.Builder.create(ProjectorBlockEntity::new, Slideshow.PROJECTOR_BLOCK).build(null));
	...
	private static Block registerBlockAndItem(String path, Block block) {
		...
		Registry.register(Registry.ITEM, new Identifier(Slideshow.ID, path), new BlockItem(block0, new Item.Settings().group(ItemGroup.MISC)));
		return block0;
	}
}