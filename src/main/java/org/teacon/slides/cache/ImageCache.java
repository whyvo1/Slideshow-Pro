...
public final class ImageCache {
	...
	private static final int MAX_CACHE_OBJECT_SIZE = 1 << 29; // 512 MB
	...
	@NotNull
	public CompletableFuture<byte[]> getResourceFromPack(@NotNull Identifier location) {
		return CompletableFuture.supplyAsync(() -> {
			MinecraftClient mc = MinecraftClient.getInstance();
			try {
				mc.getResourceManager().getResource(location);
				return IOUtils.toByteArray(mc.getResourceManager().getResource(location).getInputStream());
			} catch (IOException e) {
				LOGGER.warn(MARKER, "Failed to read bytes from resource pack.", e);
				throw new CompletionException(e);
			}
        });
	}
